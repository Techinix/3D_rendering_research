...
from models.mip_nerf_model import MipNerf
from models.mip import rearrange_render_image, distloss
from models.utils.metrics import calc_psnr
from data import dataset_dict

from models.utils.lr_schedule import MipLRDecay
from torch.utils.data import DataLoader
from models.utils.vis import stack_rgb, visualize_depth
import numpy as np
...
class MipNeRFSystem(LightningModule):
    ...
    def setup(self, stage):
        ...
        self.val_dataset = dataset(data_dir=self.hparams['data_path'],
                                   ...
                                   factor = self.hparams['val.factor']
                                   )
    ...
    def validation_step(self, batch, batch_nb):
        ...
        #save_img chw -> hwc
        img = fine_rgb[0].permute(0,1,2)
        imageio.imwrite(
                        self.hparams["out_dir"]+f"generated_images/rgb_{batch_nb}.jpg",
    ...
    def on_validation_epoch_end(self):
        mean_loss = torch.stack([x['val/loss'] for x in self.validation_step_outputs]).mean()
        mean_psnr = torch.stack([x['val/psnr'] for x in self.validation_step_outputs]).mean()
