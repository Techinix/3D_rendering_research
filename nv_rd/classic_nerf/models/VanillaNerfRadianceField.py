...
class MLP(nn.Module):
    def __init__(
            ...
            net_depth : int=8, # the depth of the network
            net_width:int=256,  # the width of the network (nb of neurons per layer)
            skip_layer:int=4, # the layer to add skip layers to (residual)
            hidden_init:Callable = nn.init.xavier_uniform_,
            hidden_activation :Callable = nn.ReLU(),
            output_enabled : bool =True,
            output_init : Callable = nn.init.xavier_uniform_,
...
class NerfMLP(nn.Module):
    def __init__(
            ...
            net_depth : int = 8 , #the depth of the network
            net_width :int =256 , #the width of the netowrk(nb of neurons per layer)
            skip_layer : int =4 , #the layer to add skip layers to
            net_depth_condtion : int = 1 , #the depth of the second part of the network 
            net_width_condition : int = 128 #the width of the second part of the network 
    ):
        super().__init__()
        self.base = MLP(
            input_dim=input_dim,
...
class VanillaNeRFRadianceField(nn.Module):
    def __init__(
        ...
        self.net_width_condtion=net_width_condition
        ...
        self.mlp = NerfMLP(
            ...
            net_depth_condtion=self.net_depth_condition,
            net_width_condition=self.net_width_condtion,
        )
