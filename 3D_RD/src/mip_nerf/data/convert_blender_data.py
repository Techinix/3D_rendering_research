...
def convert_to_nerfdata(basedir, newdir, n_down):
    ...
    for split in splits:
        ...
        if split=='x':
            ...
            step = len(data['images'])/10
            steps = np.arange(0,len(data['images']),step)
            for k in steps:
                k=int(k)
                if(k>=len(data['images'])):
                    break
                img=data['images'][k]
                for j in range(n_down):
                    fname = '{}/{:03d}_d{}.png'.format(imgdir, k, j)
                    fnames.append(fname)
                    fname = os.path.join(newdir, fname)
                    with open(fname, 'wb') as imgout:
                        img8 = Image.fromarray(np.uint8(img * 255))
                    ...
                    heights.append(img.shape[0])
                    focals.append(f / 2 ** j)
                    cam2worlds.append(data['camtoworlds'][k].tolist())
                    lossmults.append(4. ** j)
                    labels.append(j)
                    nears.append(2.)
                    fars.append(6.)
    ...
    #for split in splits:
    #    jsonfile = os.path.join(newdir, f'transforms_{split}.json')
    #    with open(jsonfile, 'w') as f:
    #        json.dump(bigmeta[split], f, ensure_ascii=False, indent=4)
