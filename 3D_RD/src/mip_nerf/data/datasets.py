...
class Blender(BaseDataset):
    ...
    def _load_renderings(self):
        ...
        if(self.split in ["train","test","val"]):
            ...
            with open(path.join(self.data_dir, 'transforms.json'), 'r') as fp:
                meta = json.load(fp)
        images = []
        cams = []
        if(not self.training):
            step = int(len(meta['frames'])/10)
            for i in range(0,len(meta['frames']),step):
                frame = meta['frames'][i]
            ...
            for i in range(0,len(meta['frames'])):
                frame = meta['frames'][i]
                fname = os.path.join(self.data_dir, frame['file_path'] + '.png')
                with open(fname, 'rb') as imgin:
                    image = np.array(Image.open(imgin), dtype=np.float32) / 255.
